...
\begin{abstract}
A polarização política nas redes sociais costuma ser estudada por redes de retweet dirigidas, que exigem conhecer o autor de cada mensagem amplificada, informação ausente em datasets desidratados. Este trabalho contorna a restrição com uma rede de co-retweet, em que dois usuários se ligam pela coincidência do que retuitam, operação equivalente ao acoplamento bibliográfico aplicado a retweets em vez de citações. A rede é construída para quatro eventos do ciclo eleitoral brasileiro de 2022, que compõem um arco de tensionamento institucional até os ataques de 8 de janeiro de 2023, com pesos dados pelo coeficiente de Jaccard e comunidades detectadas pelo algoritmo de Leiden. A análise estrutural mostra que, em torno dos eventos contenciosos, a rede se organiza de forma bipolar e fortemente assimétrica: uma comunidade se fecha de modo quase total, enquanto o lado oposto permanece poroso e subdividido. O dia do segundo turno é a exceção, com a menor segregação, e serve de controle metodológico. Os achados convergem, por um caminho independente, com estudos anteriores sobre as eleições brasileiras de 2022. A contribuição é dupla: uma pipeline reutilizável que recupera estrutura conhecida a partir de dado desidratado e a baixo custo, e a caracterização da forma da divisão ao longo do arco.
\end{abstract}

\begin{IEEEkeywords}
redes sociais, polarização política, análise de redes, detecção de comunidades, rede de co-retweet
\end{IEEEkeywords}
...
Os quatro eventos que compõem esse arco foram escolhidos por marcarem momentos sucessivos de tensionamento entre uma parcela do campo bolsonarista e as instituições. A mobilização de 7 de setembro de 2022 levou às ruas, no feriado da Independência, atos de massa com forte carga de contestação ao Supremo Tribunal Federal (STF) e ao sistema eleitoral. O caso Roberto Jefferson, em outubro, foi o momento em que essa contestação assumiu forma abertamente violenta. O dia do segundo turno, em 30 de outubro, trouxe o resultado apertado que daria origem à recusa em reconhecer a derrota. E a invasão de 8 de janeiro de 2023 foi o ápice, com a depredação das sedes dos três poderes.

O caso Roberto Jefferson merece registro mais detido, por ser o ponto do arco em que o enfrentamento com o Estado deixou de ser apenas retórico. Ex-deputado federal e ex-presidente do PTB, aliado de primeira hora de Bolsonaro, Jefferson cumpria prisão domiciliar sob acusação de integrar esquemas de milícias digitais e de praticar atos antidemocráticos. Em 23 de outubro de 2022, entre os dois turnos da eleição, agentes da Polícia Federal foram à sua casa, no interior do Rio de Janeiro, cumprir um mandado de prisão expedido pelo STF, depois que ele dirigira ofensas à ministra Cármen Lúcia. Jefferson reagiu a tiros de fuzil e com granadas, feriu dois policiais e só se entregou após horas de cerco e negociação~\cite{b_rj2022}. Um aliado proeminente respondendo a uma ordem judicial com violência armada contra agentes do Estado prefigura, em escala individual, o mesmo confronto entre uma parcela daquele campo e as instituições que se generalizaria no 8 de janeiro. É esse padrão de escalada, e não apenas a sucessão de manchetes, que justifica tratar os quatro eventos como um arco, e não como episódios isolados.

O restante deste trabalho está organizado da seguinte forma. A Seção~II situa a investigação na literatura sobre câmaras de eco, sobre a polarização no Twitter político brasileiro e na fundamentação metodológica da rede de co-retweet. A Seção~III, que concentra a contribuição da etapa aqui relatada, descreve a construção da rede e a caracterização estrutural das comunidades nos quatro eventos. A Seção~IV apresenta as conclusões e os trabalhos futuros, entre eles a caracterização ideológica e narrativa das comunidades e a aplicação de visualização interativa.
...
A pipeline transforma os arquivos CSV de um evento em um grafo de co-retweet com comunidades detectadas e métricas associadas. A Figura~\ref{fig:pipeline} resume esse percurso pela mudança da estrutura do dado, dos arquivos tabulares ao grafo de comunidades, anotando sobre cada transição a operação que a realiza. Cada módulo persiste seus artefatos em disco e os recarrega quando já existem, de modo que iterações sobre etapas tardias, tipicamente a projeção, que é a mais cara, não exijam reprocessar a carga e a filtragem dos milhões de linhas de entrada. Essa idempotência por estágio é o que torna viável a exploração de parâmetros descrita na Seção~\ref{sub:sensibilidade}, e foi implementada centralizando a política de cache em uma classe base comum a todos os módulos.

\begin{figure*}[tbp]
  \centering
  \includegraphics[width=0.96\textwidth]{fig_pipeline_v2.pdf}
  \caption{Transformação da estrutura do dado ao longo da pipeline de co-retweet, dos arquivos CSV ao grafo de comunidades. Cada nó é um estado da estrutura (tabela, matriz bipartida $B$, grafo denso, \emph{backbone} esparso e partição em comunidades) e cada seta anota a operação que o produz. A explosão combinatória de arestas causada por tweets de altíssimo alcance é tratada na etapa de projeção, com o corte de $\tau$ aplicado em blocos, e não por descarte de dados.}
  \label{fig:pipeline}
\end{figure*}
...
\begin{figure}[tbp]
  \centering
  \includegraphics[width=\columnwidth]{fig_hermeticidade.pdf}
...
Este trabalho partiu de uma intuição comum, a de que o Brasil está dividido em dois, e propôs tratá-la não como ponto de chegada, mas como ponto de partida para uma pergunta mais precisa sobre a \emph{forma} dessa divisão. A etapa aqui relatada respondeu a essa pergunta no plano estrutural, a partir da rede de co-retweet construída para quatro eventos do ciclo eleitoral de 2022.

Três fatos foram estabelecidos com base nos dados. Em torno dos eventos contenciosos analisados, a mobilização do 7 de setembro, o caso Roberto Jefferson e a invasão de 8 de janeiro, a rede se organiza de forma bipolar, com uma comunidade praticamente lacrada oposta a um lado poroso. Esse fechamento é fortemente assimétrico, de modo que os dois lados da divisão não têm o mesmo formato: um se sela quase por completo, enquanto o outro permanece permeável e, com frequência, subdividido em mais de uma comunidade. E a segregação não é constante ao longo do arco, pois o dia da eleição, momento de maior participação compartilhada, foi o de menor polarização de co-retweet, o que serviu ao mesmo tempo de contraste substantivo e de controle contra a suspeita de que o método produzisse fechamento de forma artificial.

Esses achados convergem, por um caminho metodológico independente, com a literatura sobre o Twitter político brasileiro. A comunidade hermética que aparece em torno de cada flashpoint ecoa o isolamento acentuado de uma das comunidades descrito por Interian e Rodrigues \cite{b_interian2023} para o período eleitoral, e a assimetria de fechamento reencontra a polarização assimétrica documentada por Soares, Recuero e Zago \cite{b_soares2019}. Que uma rede construída a partir de um dado mais restrito, sem acesso ao autor original dos tweets, recupere a mesma estrutura por uma operação distinta é, em si, um reforço à validade dos achados.

A contribuição desta etapa é, portanto, dupla. No plano instrumental, ela entrega uma pipeline reutilizável que recupera estrutura conhecida a partir de um dataset desidratado e a baixo custo, contornando a indisponibilidade do autor original com a projeção de co-retweet fundamentada no acoplamento bibliográfico. No plano analítico, ela caracteriza a forma da divisão em quatro momentos do arco que conduz ao 8 de janeiro. É igualmente importante demarcar o que ela não faz. A rede de co-retweet mede similaridade de consumo informacional, não fluxo de influência, e as comunidades aqui detectadas são, por ora, objetos estritamente estruturais, comunidades discursivas no sentido de Becatti et al. \cite{b_becatti2019}, e não campos ideológicos identificados. A pergunta que motivou o trabalho, sobre como dois grupos constroem leituras divergentes de um mesmo fato, depende de uma camada de conteúdo que esta etapa estabelece como possível, mas não realiza. É essa camada que organiza os trabalhos futuros descritos a seguir.

\subsection{Caracterização ideológica e narrativa das comunidades}
\label{sub:fut-caracterizacao}

A próxima etapa parte do output estrutural aqui produzido e o enriquece com o conteúdo dos tweets. Seu insumo é uma \emph{hidratação seletiva}, em que apenas um subconjunto reduzido de tweets de cada evento é recuperado via API, o suficiente para caracterizar cada comunidade sem o custo de hidratar o evento inteiro. A seleção é estratificada por bolha, e não apenas global, por uma razão empírica observada no diagnóstico preliminar do 8 de janeiro: a comunidade mais isolada é justamente a pior coberta por um ranking global de popularidade, porque consome um repertório próprio. A lista a hidratar combina, então, três critérios: os tweets mais retuitados do evento como um todo, os mais retuitados dentro de cada comunidade, e os mais específicos de cada comunidade. Esse mesmo material serve a dois propósitos que, por isso, ficam correlacionados: a rotulagem ideológica das bolhas e a análise da narrativa que cada uma amplificou.

A caracterização se apoia em dois eixos ortogonais. O primeiro é ideológico, da esquerda à direita, e é uma propriedade da \emph{conta}, atribuída a cada autor hidratado a partir do seu perfil, com classificação assistida por modelo de linguagem e revisão manual obrigatória. O segundo é de registro, do informativo ao memético, e é uma propriedade do \emph{tweet}, não da conta, já que um mesmo autor publica tanto reporte factual quanto deboche. Os dois eixos são mantidos separados porque medem coisas distintas. A análise estrutural mostrou que o lado poroso se subdivide em mais de uma comunidade, e a hipótese a verificar é que essa subdivisão não seja ideológica, mas de registro, distinguindo um polo de conteúdo jornalístico e institucional de um polo memético e irônico. Essa distinção por registro dentro de um mesmo campo reencontra a observação de Recuero, Soares e Zago \cite{b_recuero2021} de que comunidades de um mesmo lado podem se diferenciar pela qualidade do conteúdo que amplificam.

A partir da classificação dos autores, cada usuário do grafo recebe um score ideológico contínuo, calculado pela proporção entre os retweets que dirigiu a autores de direita e de esquerda,
\begin{equation}
\mathrm{score}(u) = \frac{R_{\text{dir}}(u) - R_{\text{esq}}(u)}{R_{\text{dir}}(u) + R_{\text{esq}}(u)} \in [-1, +1],
\label{eq:score}
\end{equation}
em que $R_{\text{dir}}(u)$ e $R_{\text{esq}}(u)$ são, respectivamente, o número de retweets de $u$ a autores classificados como de direita e de esquerda. O rótulo de cada comunidade é então definido pela mediana dos scores de seus membros. Esse score cumpre ainda uma função decisiva para a comparação entre eventos. O identificador numérico de um cluster não é comparável entre eventos, pois a detecção de comunidades os renumera a cada execução e a população muda. Ancorar a identidade de cada bolha na mediana do score, e não no número do cluster, permite rastrear a mesma posição ao longo do arco, de modo que ``a direita'' permaneça uma entidade reconhecível de um evento para o outro.

É neste ponto que a presente análise habilita o passo que ela própria não deu. A literatura oferece uma expectativa forte sobre a orientação da comunidade hermética: tanto o isolamento da direita descrito por Interian e Rodrigues \cite{b_interian2023} quanto a assimetria documentada por Recuero, Soares e Zago \cite{b_recuero2021} apontam o polo hiperpartidário de direita como o lado mais fechado. A rotulagem ideológica das comunidades é o que permitirá testar diretamente se o polo lacrado identificado aqui corresponde a essa expectativa, transformando uma convergência estrutural em uma afirmação sobre o conteúdo.

A operacionalização mais direta da pergunta central do trabalho, sobre duas leituras de um mesmo fato, é uma métrica de \emph{especificidade} aplicada sobre o log completo de retweets de cada evento. Para cada tweet, mede-se a fração de seus retweets que veio de cada comunidade, usando o grafo estrutural apenas como guia de pertencimento. Tweets com retweets concentrados em uma única bolha, e quase ausentes nas demais, com um piso mínimo de volume para descartar conteúdo obscuro, são as \emph{assinaturas} de cada realidade: mensagens que um lado amplificou intensamente e que o outro praticamente não viu. Recuperar essas assinaturas dá forma concreta à noção de realidades paralelas, substituindo a afirmação genérica de que os grupos veem coisas diferentes pela identificação das mensagens específicas que constituíram a leitura de cada um. Sobre os tweets-assinatura de cada par evento-bolha, uma síntese de enquadramento, também assistida por modelo de linguagem e revisada manualmente, descreverá em poucas linhas o protagonista, o antagonista e a tese central que aquela comunidade construiu sobre o evento.

\subsection{Análise longitudinal de radicalização}
\label{sub:fut-radicalizacao}

Com as comunidades rotuladas e os usuários pontuados nos quatro eventos, torna-se possível investigar a hipótese mais ambiciosa do trabalho, a de que o núcleo engajado se radicalizou ao longo do arco até o 8 de janeiro. É também a análise mais sujeita a erro, e por isso depende de controles explícitos. Antes de qualquer afirmação, um pré-teste de sobreposição de população verifica quantos usuários reaparecem de um evento para o outro, pois sem reincidência suficiente não há painel a acompanhar. A comparação precisa distinguir radicalização de mera mudança de composição, ou seja, separar o caso em que os mesmos usuários se tornaram mais extremos daquele em que apenas entraram usuários mais extremos. Por isso a medida se restringe aos usuários presentes em ao menos dois eventos e acompanha a variação do score \emph{deles}. Some-se a isso a exigência de uma régua estável, garantida por um conjunto de autores de referência classificados em todos os eventos, e um piso de ruído que exclua usuários com poucos retweets classificados, cujo score é instável. O resultado pretendido é um coeficiente de radicalização, a variação média de score do painel ao longo do arco, acompanhado de uma banda de confiança que mostre que a variação observada excede o esperado por ruído.

\subsection{Visualização interativa pública}
\label{sub:fut-visualizacao}

O último trabalho futuro é o entregável que devolve ao leitor não-técnico aquilo que a pergunta original pedia, tornar a divergência visível e navegável. Trata-se de uma aplicação web pública, construída em React com renderização de grafo via Sigma.js, que consome diretamente os artefatos das duas pipelines. Ela permitirá explorar o grafo de cada evento, com os nós coloridos por comunidade e por score, e colocar lado a lado o conteúdo amplificado por cada bolha, de modo que a noção de duas realidades sobre o mesmo fato deixe de ser uma afirmação e passe a ser uma experiência de leitura. Para o 8 de janeiro, um controle temporal mostrará a rede se preenchendo ao longo do dia. Se a análise longitudinal se confirmar, a trajetória dos usuários do painel deslizando para os extremos ao longo dos quatro eventos será a síntese visual de todo o argumento.
...
\bibitem{b_rj2022} Agência Brasil, ``Roberto Jefferson vira réu por tentativa de homicídio contra policiais,'' 9 dez. 2022. [Online]. Disponível em: https://agenciabrasil.ebc.com.br/justica/noticia/2022-12/roberto-jefferson-vira-reu-por-tentativa-de-homicidio-contra-policiais
\end{thebibliography}
